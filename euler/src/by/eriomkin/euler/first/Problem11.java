...
public class Problem11 {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < 20; i++) {
			for (int j = 0; j < 20; j++) {
				arr[i][j] = new Integer(strArr[(i * 20 + j)]);
			}
		}
		...
		for (int i = 0; i < arr.length; i++) {
			for (int j = 0; j < arr.length - 3; j++) {
				...
				if (temp > max) {
					max = temp;
					maxNumbers = "i = " + i + ", j = " + j + "; " + arr[i][j] + ", " + arr[i][j + 1] + ", "
							+ arr[i][j + 2] + ", " + arr[i][j + 3];
				}
			}
		}
		...
		for (int i = 0; i < arr.length - 3; i++) {
			for (int j = 0; j < arr.length; j++) {
				...
				if (temp > max) {
					max = temp;
					maxNumbers = "i = " + i + ", j = " + j + "; " + arr[i][j] + ", " + arr[i + 1][j] + ", "
							+ arr[i + 2][j] + ", " + arr[i + 3][j];
				}
			}
		}
		...
		for (int i = 0; i < arr.length - 3; i++) {
			for (int j = 0; j < arr.length - 3; j++) {
				...
				if (temp > max) {
					max = temp;
					maxNumbers = "i = " + i + ", j = " + j + "; " + arr[i][j] + ", " + arr[i + 1][j + 1] + ", "
							+ arr[i + 2][j + 2] + ", " + arr[i + 3][j + 3];
				}
			}
		}

		// check minor diagonal lines
		for (int i = 19; i > 3; i--) {
